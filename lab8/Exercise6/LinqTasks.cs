...
namespace Exercise6
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<Emp> result = from emp in Emps where emp.Job == "Frontend programmer" && emp.Salary > 1000 orderby emp.Ename descending select emp;
            return result;
        }
        ...
        {
            IEnumerable<Emp> result = from emp in Emps where emp.Salary == Emps.Max(emp => emp.Salary) select emp;
            return result;
        }
        ...
        {
            IEnumerable<object> result = from emp in Emps join dept in Depts on emp.Deptno equals dept.Deptno select new {emp.Ename, emp.Job, dept.Dname};
            return result;
        }
        ...
        {
            IEnumerable<object> result = from emp in Emps group emp by emp.Job into jobGroup select new {Praca = jobGroup.Key, LiczbaPracownikow = jobGroup.Count()};
            return result;
        }
        ...
        public static IEnumerable<object> Task10()
        {
            IEnumerable<object> result = Emps.Select(emp => new { emp.Ename, emp.Job, emp.HireDate })
                .Union(Emps.Select(emp =>
                {
                    return new { Ename = "Brak wartości", Job = string.Empty, HireDate = (DateTime?)null };
                }));
            return result;
        }
        ...
        public static IEnumerable<object> Task11()
        {
            IEnumerable<object> result = Depts 
                .GroupJoin(Emps, dept => dept.Deptno, emp => emp.Deptno, (dept, emp) => new { name = dept.Dname, numOfEmployees = emp.Count() })
        ...
        public static IEnumerable<Emp> Task12()
        {
            IEnumerable<Emp> result = Emps
                .Where(mgr => Emps.Where(subEmp => subEmp.Mgr == mgr).Count() > 0)
                .OrderByDescending(mgr => mgr.Salary)
                .OrderBy(mgr => mgr.Ename);
            return result;
        }
        ...
        {
            int result = arr
                .GroupBy(num => num, (key, g) => new {number = key, numOfOcurr = g.Count()})
                .Where(num => num.numOfOcurr % 2 == 1)
        ...
        public static IEnumerable<Dept> Task14()
        {
            IEnumerable<Dept> result = Depts
                .GroupJoin(
                    ...
                    dept => dept.Deptno,
                    emp => emp.Deptno,
                    (dept, emps) => new { dept, numOfEmployees = emps.Count() }
                )
                .Where(g => g.numOfEmployees == 5 || g.numOfEmployees == 0)
                .Select(g => g.dept)
